...
class GeminiService: ObservableObject {
    ...
    func scheduleFromTranscript(_ transcript: String) async throws {
        ...
        let nowString = Formatter.timeOnly.string(from: Date())
    ...
    private func parseAndInjectEvents(jsonText: String) async throws {
        ...
        let blocks = try JSONDecoder().decode([AIEventBlock].self, from: data)
        let today = Calendar.current.startOfDay(for: Date())

        var params: [CalendarManager.EventParams] = []
        for block in blocks {
            guard let time = Formatter.timeOnly.date(from: block.startTime) else { continue }
            let components = Calendar.current.dateComponents([.hour, .minute], from: time)
            if let targetDate = Calendar.current.date(byAdding: components, to: today) {
                params.append(.init(title: block.title, startDate: targetDate,
                                    durationMinutes: block.durationMinutes, notes: block.notes))
            }
        }

        await MainActor.run {
            CalendarManager.shared.insertEvents(params)
        }
    }
}
